...
{
    global $stackertable, $mysqlidb, $mysqliconnect, $mysqlidb, $mediacontrol;
    global $selectedmedia, $selectedpage, $hlcategorytable, $hlconstantstable;
...
<td class=\"small\">Logo</td></tr>";
    ...
    $querylayout = "SELECT hlconstantvalue FROM $hlconstantstable WHERE 
    hlconstants = 'elementlayout'";
...
<td><input  name=\"articlenumber\" 
onblur=\"artname(this.value,document.createarticles.position.value)\"></td>";
...
<td><input  name=\"pricekg\" id=\"createarticlespricekg\" onChange=\"checkprice(this.id)\" disabled value=\"0.00\"></td>";
    ...
    echo "</select>";
    echo "</td>";
    $logoarray = array('', 'AAA','21 Day/AAA', 'Country Kitchen', 'Floral', 
    'Ontario', 'Organic', 'Ontario/Organic', 'First of the Season', 
    'Best for the bbq','Driscoll�s','Calvo', 'Delmonte','Dole','New');
...
<td><select name=\"logo\" id=\"logo\" <option value = \"\" selected=\"\">";
    ...
    echo "</select><br />";
    echo "</td><tr>
<td ></td><td></td><td></td><td></td><td><input  name=\"articlename\" id=\"articlename\"  disabled value=\"\" </td>
<td></td><td></td><td></td><td>Delete</td><td><input  name=\"deletearticle\" type=\"checkbox\" onclick=submitarticles()></td><td>select art</td>";

    $query = "SELECT hlconstantvalue FROM $hlconstantstable 
    WHERE hlconstants = 'artselect'";
    $result = mysqli_query($mysqliconnect, $query);
